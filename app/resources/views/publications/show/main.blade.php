...
<aside id="logo-sidebar" class="fixed top-0 left-0 z-40 lg:w-1/4 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidebar">
   <div class="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800">
      ...
      <p class="text-sm text-center">{{__('Publicado el ')}} {{$publication->getFormattedCreatedAt()}}</p>
      <!-- Título -->
      <h1 class="text-2xl font-bold mb-4 text-center">{{$publication->title}}</h1>
      ...
      <p class="text-sm text-center mb-3">{{__('Días disponibles')}}</p>
      ...
      <div class="w-full text-center text-gray-900 bg-white border border-gray-200 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white">
        ...
        {{dump($dayAvailable->isAvailable())}}
            <button type="button" {{$dayAvailable->isAvailable()?'':'disabled'}} class=" {{$dayAvailable->isAvailable()?'':'cursor-not-allowed'}} relative inline-flex items-center w-full px-4 py-2 text-sm font-medium border-b border-gray-200 rounded-t-lg hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 dark:border-gray-600  dark:hover:text-gray-500 cursor-pointer dark:focus:ring-gray-500 dark:focus:text-white">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="mr-1 size-6">
               ...
               {{$dayAvailable->since . __(' hasta el ') . $dayAvailable->to . " - " . AvailableDayEnum::fromName($dayAvailable->state)}}
            </button>
        @endforeach
      </div>
   </div>
